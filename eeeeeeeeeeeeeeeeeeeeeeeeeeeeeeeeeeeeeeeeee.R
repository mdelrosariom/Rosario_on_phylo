...
all_trees <- list.files(
  path = "C:/Users/mdrmi/Downloads/Phylogenetic trees used for global analysis of island birds/Phylogenetic trees used for global analysis of island birds/Maximum clade credibility trees/",
  pattern = NULL,
  all.files = FALSE,
  full.names = FALSE
) #KNOW THE NAME OF ALL TREES

check_monophyly <- function(tree_name_file) {
  
  X_tree <- read.nexus(all_trees[1]) 
